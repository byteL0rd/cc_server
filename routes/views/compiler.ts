...
/**
 * formats and sends corresponding messsage to the user
 * 
 * @export
 ...
 * @returns 
 */
export async function formatVerifyPage(message: string, isAuth?: boolean, user?: user) {
	const _pageFormat = `${htmlHead} 
	<p class="uk-heading-primary" > Activation and Verification of User  </p>
	<div> 
		${message}
	</div>
	${htmlbody}`;
	return handlebar.compile(_pageFormat)({
		headers: _header,
		categ: await categories(),
		navbar: await navbar(isAuth, user),
	});
